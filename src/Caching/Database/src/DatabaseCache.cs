...
{
    /// <summary>
    /// Distributed cache implementation using a database.
    /// </summary>
    public class DatabaseCache : IDistributedCache
    {
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="DatabaseCache"/>.
        /// </summary>
        ...
        public DatabaseCache(ICacheDatabaseOperations databaseOperations, IOptions<DatabaseCacheOptions> options)
        {
            ...
            if (string.IsNullOrEmpty(cacheOptions.ConnectionString))
            {
                throw new ArgumentException(
                    $"{nameof(DatabaseCacheOptions.ConnectionString)} cannot be empty or null.");
            }
            if (string.IsNullOrEmpty(cacheOptions.SchemaName))
            {
                throw new ArgumentException(
                    $"{nameof(DatabaseCacheOptions.SchemaName)} cannot be empty or null.");
            }
            if (string.IsNullOrEmpty(cacheOptions.TableName))
            {
                throw new ArgumentException(
                    $"{nameof(DatabaseCacheOptions.TableName)} cannot be empty or null.");
            }
            ...
            {
                throw new ArgumentException(
                    $"{nameof(DatabaseCacheOptions.ExpiredItemsDeletionInterval)} cannot be less than the minimum " +
